...
const ListStudents = () => {
  ...
  const openModal = async (student) => {
    try {
      const response = await fetch(`https://3zn8rhvzul.execute-api.us-east-2.amazonaws.com/api/metricas-alumno/hu-tp-23/alumno/detailsClient?client_id=${student.idusuario}`);
  ...
  const filteredStudents = students.filter((student) =>
    student.alumno.toLowerCase().includes(searchTerm.toLowerCase())
  );
...
<tr key={index}>
<td className="border-t py-2 px-4 text-black">{student.alumno}</td>
...
<td className="border-t py-2 px-4 text-black">{student.membership}</td>
<td className="border-t py-2 px-4 text-black">{student.rango || 'No disponible'}</td>
...
<svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-33 h-26 text-red-600"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
>
...
<p className="mb-2"><strong>Nombres:</strong> {`${selectedStudent.nombre} ${selectedStudent.father_last_name}`}</p>
 
                <p className="mb-2"><strong>Género:</strong> {selectedStudent.genero}</p>
...
<p className="mb-2"><strong>Membresía:</strong> {selectedStudent.membership}</p>
<p><strong>Rango:</strong> {selectedStudent.rango || 'No disponible'}</p>
</div>
</div>
<div className="flex">
...
<button
                    onClick={() => navigate('/vista-no-registrado', { state: { idusuario: selectedStudent.idusuario } })}
